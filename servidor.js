...
async function criarProdutoYampi(dados) {
    ...
    // Se tem variações de tamanho
    if (dados.tamanhos.length > 1) {
        produtoData.has_variations = true;
        produtoData.variations = [];
        ...
        // Produto simples
        produtoData.has_variations = false;
        produtoData.quantity = dados.estoque[dados.tamanhos[0]] || 10;
    }
    
    log(`Criando produto na Yampi: ${dados.nome}`);
...
║              TESTE AGORA:                        ║
║  Acesse: /whatsapp                               ║
║  Digite: /cadastrar Nome: Teste Preço: R$ 19,90  ║
╚══════════════════════════════════════════════════╝
    `);
});
